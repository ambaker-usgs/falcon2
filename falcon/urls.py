...
urlpatterns = [
    ...
    re_path(r'^(falcon2/)*(?P<network>[\w+]{2})/$', views.network_level),
    re_path(r'^(falcon2/)*(?P<network>[\w+]{2})/(?P<station>\w+)/$', views.station_level),
    re_path(r'^(falcon2/)*(?P<network>[\w+]{2})/(?P<station>\w+)/(?P<channel>[a-zA-Z0-9 ]+)/$', views.channel_level, name='channel_level'),
